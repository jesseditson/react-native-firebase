...
export default class Auth extends Base {
  ...
  signInWithPhoneNumber(phoneNumber: string): Promise<Object> {
    return this._interceptUserValue(FirebaseAuth.signInWithPhoneNumber(phoneNumber));
  }

  /**
   * Listen for auth code sent events
   * @param handler
   * @return {Function}   An unsubscribe function to call when no more events are needed.
   */
  onAuthCodeSent(handler: verificationID => void) {
    this.log.info('Creating onAuthCodeSent listener');
    const listener = ({verificationID}: {verificationID: string}) => handler(verificationID);
    FirebaseAuthEvt.addListener('onAuthCodeSent', listener);
    return () => {
      this.log.info('Removing onAuthCodeSent listener');
      FirebaseAuthEvt.removeListener('onAuthCodeSent', listener);
    };
  }
  ...
  verifyCode(verificationID: string, verificationCode: string) {
    return FirebaseAuth.verifyCode(verificationID, verificationCode);
  }
